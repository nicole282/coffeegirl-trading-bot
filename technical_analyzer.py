...
class TechnicalAnalyzer:
    ...
    def analyze_trend(self, prices: List[float]) -> Dict:
        ...
        # 计算多个时间段的收益率
        returns_5 = (prices[-1] / prices[-5] - 1) if len(prices) >= 5 else0
    ...
    def comprehensive_analysis(self, prices: List[float]) -> Dict:
        ...
        if rsi < self.config.RSI_OVERSOLD:
            buy_signals += 1
            total_confidence += (self.config.RSI_OVERSOLD - rsi) / self.config.RSI_OVERSOLD * 1.5
        elif rsi > self.config.RSI_OVERBOUGHT:
            sell_signals += 1
            total_confidence += (rsi - self.config.RSI_OVERBOUGHT) / (100 - self.config.RSI_OVERBOUGHT) * 1.5
        ...
        if trend_analysis["trend"] == "UPTREND":
            buy_signals += 1
            total_confidence += min(0.5, trend_analysis["strength"] / 5)
